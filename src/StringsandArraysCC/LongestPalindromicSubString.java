...
public class LongestPalindromicSubString {
	...
	private int lps_Aux(Integer[][] dp, String string, int startIndex, int endIndex) {
		...
		if (dp[startIndex][endIndex] == null) { //If we have not solved this problem previously, only then solve it
			if (string.charAt(startIndex) == string.charAt(endIndex)) {
				int remlen = endIndex - startIndex - 1;
				if(remlen == lps_Aux(dp, string, startIndex + 1, endIndex - 1))
				c1 =remlen + 2;//CASE#1 - If index pointed characters matches then we add 2 to the existing known palindrome length
			} 
	...
	public static void main(String[] args) {
		//input - 'ABCYRCFBTH', pallindromic substring 'A'(we cannot ignore any of the character), pallindromic subsequence 'ABCYCBA'
